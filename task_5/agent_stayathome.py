    ...
    def get_action(self, obs: dict) -> dict:
        ...
        for ship in obs['allied_ships']:
            ...
            if move_cooldown == 0:
                ...
                #     planet_x, planet_y, occupation_progress = planet
                #     # print(planet_x,planet_y, occupation_progress)
                #
                #     # If planet is unoccupied
                #     if occupation_progress == -1:
                #         # Calculate direction to move towards the planet
                #         direction = self.calculate_direction(ship_x, ship_y, planet_x, planet_y)
                #         break
                # else: # for' else:
                # direction = self.calculate_direction(ship_x, ship_y, self.step_direction_x, self.step_direction_y)

                speed = 1  # Move 1 step at a time
                if ship_id != 0:
                    direction = self.calculate_direction(ship_x, ship_y, self.home_base_x, self.home_base_y)
                else:
                    direction = 3
